...
(function (D) {
    ...
    // default params
        ...
        _cityID = 0,
    //configures
        zone = 'www.dianping.com',
        netSpeedSampleRate = 0.25, //测网速的采样率
    // downloadFileURL = 'http://1.ispeed.sinaapp.com/test381.zip', //测试目标文件
    // swfFileURL = 'http://1.ispeed.sinaapp.com/SpeedTest.swf', //测试用的核心文件
        downloadFileURL = 'http://i1.dpfile.com/s/res/test381.zip',
        swfFileURL = 'http://i1.dpfile.com/s/res/speedtest.swf',
    ...
    //URL_BASE = 'http://hls.' + ( /51ping/.test(doc.domain) ? '51ping' : 'dianping' ) + '.com/hippo.gif?',
        URL_BASE = 'http://hls.' + ( /dianping\.com/.test(doc.domain) ? 'dianping' : '51ping' ) + '.com/hippo.gif?',
    // URL_BASE_MONGO_DB = 'http://hls.dianping.com/salmon.gif?',

        SCREEN_SIZE_KEY = '__hsr',
        SCREEN_COLOR_DEPTH_KEY = '__hsc',
        FLASH_VERSION_KEY = '__hfv',
        NET_SPEED_KEY = 'r_speed',
        PAGE_TIMING_KEY = 'r_pagetiming',

        WAITING_KEY = 'r_wait',
        CONNECTING_KEY = 'r_conn',
        RESPONSE_END_KEY = 'r_recev',

    //ZONE_KEY = '__hlt',
        LOCATION_HREF_KEY = '__hlh',
        LOCATION_REFERRER_KEY = '__hlr',
    // METHOD_KEY = '__hmet',  // 代码中未发现使用
    // ARGS_KEY = '__harg',
    //ATTACH_KEY = '__had',
    ...
    // PAGE_ID_KEY = '__ppp',
         ...
         * optimized for v8
         * @param obj {Object} 需要字符串化的对象
         * @returns {String}
         */
            stringify = (function () {
            var sy = (typeof JSON !== 'undefined') && JSON.stringify;
            return sy ? sy : function (obj) {
                var ret = [], _e = encode;

                for (var key in obj) {
                    ret.push('"' + _e(key) + '":"' + _e(("" + obj[key]).replace(/"/g, '\\"')) + '"');
                }
         ...
         */
            ...
            var h, w, s, c, f,
                __presets = {}; //default parameters for hippo request
            loc && loc.href && (__presets[LOCATION_HREF_KEY] = loc.href);
            ref && (__presets[LOCATION_REFERRER_KEY] = ref);

            if (s = screen) {
                h = s.height;
                w = s.width;
                if (h && w) {
                    __presets[SCREEN_SIZE_KEY] = w + 'x' + h;
                }
                if (c = s.colorDepth) {
                    __presets[SCREEN_COLOR_DEPTH_KEY] = c + 'bit';
                }
            }
            ;
            (f = getFlashVersion()) && (__presets[FLASH_VERSION_KEY] = f);

            return function (key, value) {
                var current = {
                    '__hlt':zone,
                    '__ppp':_pageID,
                    '__had':stringify(data_attached),
                    'force':(+new Date)
                };

                isArray(value) && (value = value.join('|'));
                checkKeyValue(key, value) && (current[key] = value);
                return toQueryString(mix(current, __presets));
            };
        })();

    function encode(text, nullable) {
        return nullable ? encodeURI(text) : encodeURIComponent(text);
    };
    function getFlashVersion() {
        var version = UA.getFlashVersion();
        if (version) {
    ...
    function checkKeyValue(key, value) {
        var v = value;
        return isString(key) && ((isNumber(v) && !isNaN(v)) || isString(v) || isBoolean(v));
    };
    /**
    ...
    function send(key, value) {
        var query = generateQuery(key, value);
        // new Image(1, 1).src = URL_BASE_MONGO_DB + query;
        new Image(1, 1).src = URL_BASE + query;
    };
    /**
     *
     * @param page{Number} id of current page
     * @param z {String}(optional) base url of site
     */
    var hippo = function (page, z) {
        if (!__inited) {
            __inited = true;
            if (z) zone = z;
            _pageID = page;
    ...
    function parseCommand(command) {
        ...
        if (isArray(it) && it.length) {
            ret = function () {
                self[it.splice(0, 1)[0]].apply(self, it);
            }
    ...
    function executeCommandQueue() {
        ...
        while (len--) {
            it = q.splice(0, 1);
            command = parseCommand.call(self, it[0]);
            try {
                command();
            } catch (e) {
                failed++;
    ...
    function targetSample(rate) {
        return Math.random() <= rate;
    }

    function collectPerformance(ext) {
//        D.provide('io/swiff', function (D, Swiff) {
//            var containerId = 'swfContainer_netspeed';
//            $.create('div').attr('id', containerId).css({'height':'1px', 'width':'1px', 'overflow':'hidden'}).inject($(doc.body));
//            new Swiff(swfFileURL, '#' + containerId, {
//                cached:false,
//                flashVars:{
//                    downloadURL:downloadFileURL + '?nocache=' + Math.random(),
//                    debugOn:false
//                }
//            }).on({
//                downloaded:function (data) {
//                    ext[NET_SPEED_KEY] = data.totalBytes * 1000 / (data.time * 1024);
//                    if(D.data('versionId')) ext['version']=D.data('versionId');
//                    hippo.ext(ext).mv(PAGE_TIMING_KEY);
//                }
//            });
...
//        });
    ...
    if (isArray(__hippo) && __hippo.length) {
        __commandQueue = [].concat(__hippo);
    }
    ...
    mix(hippo, {
        ...
        ext:function (name, value) {
            if (isObject(name)) {
                for (var key in name) {
                    hippo.ext(key, name[key]);
                }

            } else if (checkKeyValue(name, value)) {
         ...
         * @param name{String} optional, if no name passed in, hippo will remove all relative data
         * @return {Hippo}
         */
        rxt:function (name) {
            if (isString(name)) {
         ...
         * @param cityID {Number}
         * @param channelID {Number} (optional)
         * @param maintainExtData {Boolean}(optional) 是否保留通过 ext 方法添加的数据，默认为不保留
         */
        pv:function (cityID, channelID, maintainExtData) {
            if (isNumber(cityID)) {
                send(PAGE_TRACK_KEY, [_cityID = cityID, _channelID = channelID >>> 0 || 0]);
                !maintainExtData && hippo.rxt();
            }
            return hippo;
        },
        /**
         ...
         * @param value{*}
         * @param maintainExtData{Boolean}(optional)
         */
        mv:function (moduleId, value, maintainExtData) {
            if (isString(moduleId)) {
                send(MODULE_TRACK_KEY, [moduleId, value || '', _cityID, _channelID]);
                !maintainExtData && hippo.rxt();
            }
            return hippo;
        },
        /**
         *
         * @param cmmand {Array||Function}
         */
        push:function (command) {
            var cs = command;

            if (isArray(cs) && cs.length || isFunction(cs)) {
                __commandQueue.push(cs);
            } else {
                return false;
            }
            return executeCommandQueue.call(hippo) || 0;
        },
         ...
         * test net speed
         */
        _pageTiming:function (r_load, r_ready) {
            if (targetSample(netSpeedSampleRate)) {
                var perf = win.performance,
                    t = perf && perf.timing || {},
                    domLoading = t.domLoading,

                    connect = t.connectEnd - t.connectStart,
                    recev = t.responseEnd - t.responseStart,
                    domready = (t.domInteractive - t.domLoading) || r_ready,
                    waiting = t.responseStart - t.requestStart,
                    loaded = (t.loadEventStart - domLoading) || r_load;

                collectPerformance({'r_load':loaded, 'r_ready':domready, 'r_wait':waiting, 'r_recev':recev, 'r_conn':connect});
            }
            return hippo;
        },
        ...
        /**
         * set sample rate of net speed case
         * @param rate {Number}
         */
        _setNetSpeedSampleRate:function (rate) {
            netSpeedSampleRate = Math.max(Math.min(1, parseFloat(rate)), 0);
            return hippo;
        },
        _setNetSpeedDownloadFileURL:function (url) {
            downloadFileURL = url;
            return hippo;
        },
        _setNetSpeedSwfFileURL:function (url) {
            swfFileURL = url;
            return hippo;
        }
    });
    ...
    DP.ready(function (D) {
        ...
        if (timeStart = win.G_rtop) {
            var h = document.hippo, r_ready = +new Date - timeStart;
            $(win).on('load', function () {
                _hip.push(['_pageTiming', +new Date - timeStart, r_ready]);
            });
        }
    });

    win[DPGA_HOST_NAME] = doc.hippo = hippo;
})(DP);
